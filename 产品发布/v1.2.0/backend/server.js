...
const { createProject, createDefaultOrgData, createDefaultFlowTemplates, ROLE_FAMILY_DEFAULTS } = require("./default-data");
const {
  findFlowTemplate,
  findFlowInstance,
  createFlowTemplate,
  ...
  return async function handleRequest(request, response) {
    ...
    try {
      ...
      if (segments[0] === "api" && segments[1] === "flow-templates" && segments.length === 3 && request.method === "GET") {
        ...
        if (!template) { sendError(response, 404, "流程模板不存在"); return; }
      ...
      if (segments[0] === "api" && segments[1] === "flow-templates" && segments.length === 3 && request.method === "PATCH") {
        ...
        try {
          ...
          if (result._missing) { sendError(response, 404, "流程模板不存在"); return; }
          sendJson(response, 200, { data: result });
        } catch (error) {
          sendError(response, 400, error.message);
        }
      ...
      if (segments[0] === "api" && segments[1] === "flow-templates" && segments.length === 3 && request.method === "DELETE") {
        const result = store.update((data) => {
          const outcome = deleteFlowTemplate(data, segments[2]);
          if (!outcome) return { _missing: true };
          if (outcome._error) return outcome;
          store.audit("flow-template.delete", `删除流程模板`, { templateId: segments[2] });
          return outcome;
        });
        if (result._missing) { sendError(response, 404, "流程模板不存在"); return; }
        if (result._error) { sendError(response, 409, result._error, { usages: result._usages }); return; }
        sendJson(response, 200, { data: result });
        return;
      }

      // POST /api/flow-templates/:id/clone
      if (segments[0] === "api" && segments[1] === "flow-templates" && segments[3] === "clone" && segments.length === 4 && request.method === "POST") {
        const cloned = store.update((data) => {
          const result = cloneFlowTemplate(data, segments[2]);
          if (!result) return { _missing: true };
          store.audit("flow-template.clone", `复制流程模板：${result.name}`, { templateId: result.id, sourceId: segments[2] });
          return result;
        });
        if (cloned._missing) { sendError(response, 404, "流程模板不存在"); return; }
      ...
      if (url.pathname === "/api/flow-instances" && request.method === "GET") {
        const data = store.snapshot();
        const { projectId, status } = url.searchParams;
      ...
      if (url.pathname === "/api/flow-instances" && request.method === "POST") {
        const body = await readJson(request);
        try {
          const instance = store.update((data) => {
            const result = createFlowInstance(data, body.projectId, body.templateId);
            store.audit("flow-instance.create", `创建流程实例：${result.templateName} → 项目 ${body.projectId}`, { instanceId: result.id, projectId: body.projectId, templateId: body.templateId });
            return result;
          });
      ...
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments.length === 3 && request.method === "GET") {
        ...
        if (!instance) { sendError(response, 404, "流程实例不存在"); return; }
        sendJson(response, 200, { data: instance });
        return;
      }

      // PATCH /api/flow-instances/:id
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments.length === 3 && request.method === "PATCH") {
        const body = await readJson(request);
        const result = store.update((data) => {
          const instance = updateFlowInstance(data, segments[2], body);
          if (!instance) return { _missing: true };
          store.audit("flow-instance.update", `更新流程实例状态：${instance.status}`, { instanceId: instance.id });
          return instance;
        });
        if (result._missing) { sendError(response, 404, "流程实例不存在"); return; }
        sendJson(response, 200, { data: result });
        return;
      }

      // DELETE /api/flow-instances/:id
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments.length === 3 && request.method === "DELETE") {
        const result = store.update((data) => {
          const outcome = deleteFlowInstance(data, segments[2]);
          if (!outcome) return { _missing: true };
          store.audit("flow-instance.delete", `删除流程实例`, { instanceId: segments[2] });
          return outcome;
        });
        if (result._missing) { sendError(response, 404, "流程实例不存在"); return; }
        sendJson(response, 200, { data: result });
      ...
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments[3] === "advance" && segments.length === 4 && request.method === "POST") {
        try {
          const result = store.update((data) => {
            const instance = advanceFlowInstance(data, segments[2]);
            if (!instance) return { _missing: true };
            store.audit("flow-instance.advance", `推进流程实例到下一阶段`, { instanceId: instance.id, currentStageId: instance.currentStageId, status: instance.status });
            return instance;
          });
          if (result._missing) { sendError(response, 404, "流程实例不存在"); return; }
          sendJson(response, 200, { data: result });
      ...
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments[3] === "review" && segments.length === 4 && request.method === "POST") {
        const body = await readJson(request);
        try {
          const result = store.update((data) => {
            const instance = reviewGate(data, segments[2], body);
            if (!instance) return { _missing: true };
            store.audit("flow-instance.review", `关口评审：${body.action}`, { instanceId: instance.id, action: body.action });
            return instance;
          });
          if (result._missing) { sendError(response, 404, "流程实例不存在"); return; }
          sendJson(response, 200, { data: result });
      ...
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments[3] === "return" && segments.length === 4 && request.method === "POST") {
        const body = await readJson(request);
        try {
          const result = store.update((data) => {
            const instance = returnStage(data, segments[2], body);
            if (!instance) return { _missing: true };
            store.audit("flow-instance.return", `退回流程阶段`, { instanceId: instance.id, reason: body.reason });
            return instance;
          });
          if (result._missing) { sendError(response, 404, "流程实例不存在"); return; }
          sendJson(response, 200, { data: result });
      ...
      // GET /api/flow-instances/:id/stages/:stageId
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments[3] === "stages" && segments.length === 5 && request.method === "GET") {
        const data = store.snapshot();
        const instance = findFlowInstance(data, segments[2]);
        if (!instance) { sendError(response, 404, "流程实例不存在"); return; }
        const stage = instance.stages.find((s) => s.id === segments[4]);
        if (!stage) { sendError(response, 404, "阶段不存在"); return; }
        sendJson(response, 200, { data: { instance, stage } });
        return;
      }

      // PATCH /api/flow-instances/:id/stages/:stageId
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments[3] === "stages" && segments.length === 5 && request.method === "PATCH") {
        const body = await readJson(request);
        const result = store.update((data) => {
          const outcome = updateFlowInstanceStage(data, segments[2], segments[4], body);
          if (!outcome) return { _missing: true };
          return outcome;
        });
        if (result._missing) { sendError(response, 404, "流程实例或阶段不存在"); return; }
        sendJson(response, 200, { data: result });
        return;
      }

      // POST /api/flow-instances/:id/stages/:stageId/receipts
      if (segments[0] === "api" && segments[1] === "flow-instances" && segments[3] === "stages" && segments[5] === "receipts" && segments.length === 6 && request.method === "POST") {
        const body = await readJson(request);
        try {
          const result = store.update((data) => {
            const outcome = submitStageReceiptToFlow(data, { instanceId: segments[2], stageId: segments[4], ...body });
            store.audit("flow-instance.receipt", `阶段回执：${body.status}`, { instanceId: segments[2], stageId: segments[4], status: body.status });
            return outcome;
          });
          sendJson(response, 200, { data: result });
        } catch (error) {
